...
#tf.compat.v1.disable_eager_execution()
    input_tensor = tf.placeholder(tf.int32,shape=(128,), name='input-0')
...
#input_tensor = tf.constant([1, 2, 3], dtype=tf.int64)
    ...
    values = tf.constant(np.array([4, 5, 6, 7, 8, 9]),  dtype=tf.int32, name='values')
    ...
    out0 = table.lookup(input_tensor)
...
data = np.random.uniform(1, 9, size=(128,)).astype("int32")
#print(data)
...
writefile_ = open("tvm_graph_def","w")
